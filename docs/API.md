...
보완 요청은 기존 신청서와 자동저장 초안을 초기화하지 않습니다. 교직원이 선택한 `revisionItems`에 해당하는 입력칸을 학생 화면에서 수정할 수 있습니다.
요청 항목으로 바로 이동하며 기존 입력값을 유지합니다. 보완 요청 상태에서는
서명본 요청 여부와 관계없이 제출 파일의 교체·삭제도 가능합니다.
...
신청 화면은 아래 단계로 구성됩니다. React의 현재 화면 상태와
...
`SUBMITTED`, `APPROVED` 및 과거 호환용 `REJECTED` 상태에서는 업로드·교체·삭제가 거부되지만 다운로드는
가능합니다.
...
`GET /api/staff/crawling/latest`는 저장된 최신 Snapshot을 조회합니다. 공지, 일정별 추출 근거, 첨부파일 분석 상태, 연구 주제, 경고·오류를 반환합니다. 새 크롤링이나 Database 재적재는 실행하지 않습니다.

## 학적 정보와 상태

현재 신청서의 `student.grade`는 `users.grade`에서 가져옵니다. 학년도·학기는 연결된 과목의 공지에서 조회합니다. 학년은 학번으로 추정하지 않습니다.

현재 교직원 처리 동작은 승인과 보완 요청입니다. 반려 버튼·필터·집계 및 반려 실행 API는 제공하지 않으며, `REJECTED`는 기존 데이터 호환을 위한 상태로만 남아 있습니다.
