...
| 단계 | URL suffix | 역할 |
| --- | --- | --- |
| S2 | `/applicant` | 신청자 정보 및 연락처 확인 |
| S3 | `/content` | 신청사유·연구목적·상세 작성 |
| S4 | `/documents` | 신청서 PDF 및 HWPX 생성·다운로드 |
...
| 신청서 작성 | 진행 완료 | 상세 항목 입력, debounce 자동저장, HWPX/PDF 생성 |
| 교직원 검토 | 예정 | 신청 목록, 상세 검토, 승인/보완 요청/반려 |
| HWPX 템플릿 | 진행 완료 | 업로드, 구조·placeholder 검증, 활성화, 다운로드 |
| 생성 문서 | 진행 완료 | HWPX/PDF 생성, 메타데이터 저장, 권한 기반 다운로드 |
